# Description: Domain service registry builder for Spectra LS setup-entry wiring.
...
ServiceStage = tuple[str, Any, tuple[Any, ...], dict[str, Any]]


async def _run_service_sequence(sequence_label: str, stages: list[ServiceStage]) -> None:
...
def _call_text(call: ServiceCall, key: str, default: str = "") -> str:
    return str(call.data.get(key, default))


def _call_clean_text(call: ServiceCall, key: str, default: str = "") -> str:
    return str(call.data.get(key, default) or "").strip()


def _call_optional_clean_text(call: ServiceCall, key: str) -> str | None:
    value = _call_clean_text(call, key)
...
def _call_bool(
    ...
    key: str,
    default: bool,
    coerce_bool: Callable[[object, bool], bool],
) -> bool:
    return coerce_bool(call.data.get(key), default=default)
...
def _call_int(
    ...
    return coerce_int(call.data.get(key), default=default)


def _build_authority_pipeline_stages(
    *,
    coordinator: Any,
    mode: str,
    reason: str,
    include_selection_handoff: bool = False,
    include_metadata_prep: bool = False,
    include_capability_profile: bool = False,
    include_action_catalog: bool = False,
    include_crossfade_balance: bool = False,
) -> list[ServiceStage]:
    ...
    if include_selection_handoff:
        stages.append(("validate_selection_handoff", coordinator.async_validate_selection_handoff, (), {}))
    if include_metadata_prep:
        stages.append(("validate_metadata_prep", coordinator.metadata_stack.async_validate_metadata_prep, (), {}))
    if include_capability_profile:
        stages.append(("validate_capability_profile", coordinator.async_validate_capability_profile, (), {}))
    if include_action_catalog:
        stages.append(("validate_action_catalog", coordinator.async_validate_action_catalog, (), {}))
    if include_crossfade_balance:
        stages.append(("validate_crossfade_balance", coordinator.async_validate_crossfade_balance, (), {}))

    return stages


def _parse_scheduler_choice_request(
    *,
    call: ServiceCall,
    coerce_bool: Callable[[object, bool], bool],
    coerce_int: Callable[[object, int], int],
    include_apply_controls: bool,
) -> dict[str, Any]:
    parsed: dict[str, Any] = {
        "require_control_capable": _call_bool(
            call,
            "require_control_capable",
            default=True,
            coerce_bool=coerce_bool,
        ),
        "prefer_fresh": _call_bool(
            call,
            "prefer_fresh",
            default=True,
            coerce_bool=coerce_bool,
        ),
        "max_results": max(1, _call_int(call, "max_results", default=5, coerce_int=coerce_int)),
        "target_hint": _call_clean_text(call, "target_hint"),
        "correlation_id": _call_optional_clean_text(call, "correlation_id"),
    }
    if include_apply_controls:
        parsed["dry_run"] = _call_bool(call, "dry_run", default=True, coerce_bool=coerce_bool)
        parsed["force"] = _call_bool(call, "force", default=False, coerce_bool=coerce_bool)
    return parsed


def _parse_scaffold_control_request(
    *,
    call: ServiceCall,
    coerce_bool: Callable[[object, bool], bool],
    include_none_default: bool = True,
) -> dict[str, Any]:
    return {
        "dry_run": _call_bool(call, "dry_run", default=True, coerce_bool=coerce_bool),
        "force": _call_bool(call, "force", default=False, coerce_bool=coerce_bool),
        "include_none": _call_bool(
            call,
            "include_none",
            default=include_none_default,
            coerce_bool=coerce_bool,
        ),
        "correlation_id": _call_optional_clean_text(call, "correlation_id"),
    }
...
def build_service_registrations(
    *,
    hass: HomeAssistant,
    entry: ConfigEntry,
    coordinator: Any,
    default_authority_mode: Callable[[], str],
    coerce_bool: Callable[[object, bool], bool],
    coerce_int: Callable[[object, int], int],
    build_authority_snapshot_summary: Callable[[dict[str, Any]], dict[str, Any]],
    build_host_cutover_snapshot_summary: Callable[[dict[str, Any]], dict[str, Any]],
) -> tuple[tuple[str, Any, SupportsResponse | None], ...]:
    """Build domain service registrations for Spectra LS entry services."""
    ...
    async def _service_set_write_authority(call: ServiceCall) -> None:
        mode = _call_text(call, "mode", default_authority_mode())
        reason = _call_text(call, "reason", "")
    ...
    async def _service_metadata_write_trial(call: ServiceCall) -> None:
        mode = _call_text(call, "mode", default_authority_mode())
        window_id = _call_clean_text(call, "window_id")
        reason = _call_clean_text(call, "reason")
        dry_run = _call_bool(call, "dry_run", default=True, coerce_bool=coerce_bool)
        expected_target = _call_optional_clean_text(call, "expected_target")
        expected_route = _call_optional_clean_text(call, "expected_route")
        expected_meta_entity = _call_optional_clean_text(call, "expected_meta_entity")
        correlation_id = _call_optional_clean_text(call, "correlation_id")
    ...
    async def _service_run_p3_s01_sequence(call: ServiceCall) -> None:
        mode = _call_text(call, "mode", WRITE_AUTH_COMPONENT)
        reason = _call_text(call, "reason", "")
        correlation_id = _call_optional_clean_text(call, "correlation_id")
        force = _call_bool(call, "force", default=False, coerce_bool=coerce_bool)

        stages = _build_authority_pipeline_stages(
        ...
        stages.append(
            (
                "route_write_trial",
                coordinator.async_route_write_trial,
                (),
                {"correlation_id": correlation_id, "force": force},
            )
        )
        await _run_service_sequence("P3-S01", stages)

    async def _service_run_p3_s02_sequence(call: ServiceCall) -> None:
        mode = _call_text(call, "mode", WRITE_AUTH_COMPONENT)
        reason = _call_text(call, "reason", "")
        run_write_trial = _call_bool(call, "run_write_trial", default=False, coerce_bool=coerce_bool)
        correlation_id = _call_optional_clean_text(call, "correlation_id")
        force = _call_bool(call, "force", default=False, coerce_bool=coerce_bool)
        ...
        stages = _build_authority_pipeline_stages(
            ...
            mode=mode,
            reason=reason,
            include_selection_handoff=True,
        )
        if run_write_trial:
            stages.insert(
                len(stages) - 1,
        ...
        await _run_service_sequence("P3-S02", stages)
    ...
    async def _service_validate_metadata_policy(call: ServiceCall) -> None:
        ...
        trigger_provider_refresh = _call_bool(
            call,
        ...
        refresh_reason = (
            _call_clean_text(call, "refresh_reason", "validate_metadata_policy")
            or "validate_metadata_policy"
        )
        force_refresh = _call_bool(call, "force_refresh", default=True, coerce_bool=coerce_bool)
    ...
    async def _service_get_authority_contract(call: ServiceCall) -> dict[str, Any]:
        refresh = _call_bool(call, "refresh", default=True, coerce_bool=coerce_bool)
        include_snapshot_summary = _call_bool(
            call,
            "include_snapshot_summary",
            default=False,
            coerce_bool=coerce_bool,
        )
        fail_closed_if_unready = _call_bool(
            call,
            "fail_closed_if_unready",
            default=False,
            coerce_bool=coerce_bool,
        )
        fail_closed_if_contract_not_ready = _call_bool(
            call,
            "fail_closed_if_contract_not_ready",
            default=False,
            coerce_bool=coerce_bool,
        )
        required_checkpoint_count = _call_int(
            call,
        ...
        if include_snapshot_summary:
            return {
                ...
                "snapshot_summary": build_authority_snapshot_summary(snapshot),
            }
    ...
    async def _service_get_host_cutover_gate(call: ServiceCall) -> dict[str, Any]:
        ...
        fail_closed_if_not_ready = _call_bool(
            call,
            "fail_closed_if_not_ready",
            default=False,
            coerce_bool=coerce_bool,
        )
        fail_closed_if_not_activation_ready = _call_bool(
            call,
            "fail_closed_if_not_activation_ready",
            default=False,
            coerce_bool=coerce_bool,
        )
        ...
        if fail_closed_if_not_ready and not bool(host_gate.get("ready_for_cutover", False)):
            blocker_count = (
                len(host_gate.get("gate_blockers", []))
                if isinstance(host_gate.get("gate_blockers", []), list)
                else 0
            )
            raise HomeAssistantError(
        ...
        if fail_closed_if_not_activation_ready and not bool(
            ...
            blocker_count = (
                len(host_gate.get("activation_blockers", []))
                if isinstance(host_gate.get("activation_blockers", []), list)
                else 0
            )
            raise HomeAssistantError(
        ...
        if include_snapshot_summary:
            return {
                ...
                "snapshot_summary": build_host_cutover_snapshot_summary(snapshot),
            }
    ...
    async def _service_run_p3_s03_sequence(call: ServiceCall) -> None:
        ...
        stages = _build_authority_pipeline_stages(
            ...
            reason=reason,
            include_selection_handoff=True,
            include_metadata_prep=True,
        )
        await _run_service_sequence("P3-S03", stages)

    async def _service_run_p5_s02_sequence(call: ServiceCall) -> None:
        mode = _call_text(call, "mode", default_authority_mode())
        reason = _call_text(call, "reason", "P5-S02 one-shot sequence")
        window_id = _call_clean_text(call, "window_id")
        dry_run = _call_bool(call, "dry_run", default=True, coerce_bool=coerce_bool)
        expected_target = _call_optional_clean_text(call, "expected_target")
        expected_route = _call_optional_clean_text(call, "expected_route")
    ...
    async def _service_run_f4_s01_sequence(call: ServiceCall) -> None:
        ...
        stages = _build_authority_pipeline_stages(
            ...
            include_selection_handoff=True,
            include_metadata_prep=True,
            include_capability_profile=True,
        )
    ...
    async def _service_run_f4_s03_sequence(call: ServiceCall) -> None:
        ...
        stages = _build_authority_pipeline_stages(
            ...
            include_capability_profile=True,
            include_action_catalog=True,
            include_crossfade_balance=True,
        )
    ...
    async def _service_run_scheduler_choice(call: ServiceCall) -> None:
        parsed = _parse_scheduler_choice_request(
            call=call,
            coerce_bool=coerce_bool,
            coerce_int=coerce_int,
            include_apply_controls=False,
        )
        await coordinator.async_run_scheduler_choice(**parsed)
    ...
    async def _service_apply_scheduler_choice(call: ServiceCall) -> None:
        parsed = _parse_scheduler_choice_request(
            ...
            coerce_bool=coerce_bool,
            coerce_int=coerce_int,
            include_apply_controls=True,
        )
        await coordinator.async_apply_scheduler_choice(**parsed)

    async def _service_build_target_options_scaffold(call: ServiceCall) -> None:
        parsed = _parse_scaffold_control_request(call=call, coerce_bool=coerce_bool)
        await coordinator.async_build_target_options_scaffold(**parsed)

    async def _service_run_auto_select_scaffold(call: ServiceCall) -> None:
        parsed = _parse_scaffold_control_request(call=call, coerce_bool=coerce_bool)
        sync_options_if_missing = _call_bool(
            call,
            "sync_options_if_missing",
            default=False,
            coerce_bool=coerce_bool,
        )
        await coordinator.async_run_auto_select_scaffold(
            ...
            include_none=parsed["include_none"],
    ...
    async def _service_run_metadata_trial_bridge_scaffold(call: ServiceCall) -> None:
        window_id = _call_clean_text(call, "window_id")
        reason = _call_clean_text(call, "reason")
        resolver_dry_run = _call_bool(call, "resolver_dry_run", default=True, coerce_bool=coerce_bool)
        trial_dry_run = _call_bool(call, "trial_dry_run", default=True, coerce_bool=coerce_bool)
        force = _call_bool(call, "force", default=False, coerce_bool=coerce_bool)
        expected_target = _call_optional_clean_text(call, "expected_target")
        expected_route = _call_optional_clean_text(call, "expected_route")
        expected_meta_entity = _call_optional_clean_text(call, "expected_meta_entity")
    ...
    async def _service_set_active_target(call: ServiceCall) -> None:
        target = _call_clean_text(call, "target")
        dry_run = _call_bool(call, "dry_run", default=False, coerce_bool=coerce_bool)
        force = _call_bool(call, "force", default=False, coerce_bool=coerce_bool)
        sync_options_if_missing = _call_bool(
            call,
            "sync_options_if_missing",
    ...
    async def _service_set_metadata_override(call: ServiceCall) -> None:
        enable = _call_bool(call, "enable", default=False, coerce_bool=coerce_bool)
        entity_id = _call_optional_clean_text(call, "entity_id")
        dry_run = _call_bool(call, "dry_run", default=False, coerce_bool=coerce_bool)
        force = _call_bool(call, "force", default=False, coerce_bool=coerce_bool)
        reason = _call_clean_text(call, "reason")
        correlation_id = _call_optional_clean_text(call, "correlation_id")
    ...
    async def _service_set_metadata_provider_packet(call: ServiceCall) -> None:
        status = _call_text(call, "status", "")
        response = _call_text(call, "response", "")
        providers = _call_text(call, "providers", "")
        item_uri = _call_text(call, "item_uri", "")
        reason = _call_text(call, "reason", "")
        updated_at = _call_text(call, "updated_at", "")
        source = _call_text(call, "source", "")
    ...
    async def _service_execute_control_center_input(call: ServiceCall) -> None:
        input_event = _call_clean_text(call, "input_event")
        correlation_id = _call_optional_clean_text(call, "correlation_id")
        target_hint = _call_optional_clean_text(call, "target_hint")
        dry_run = _call_bool(call, "dry_run", default=False, coerce_bool=coerce_bool)
