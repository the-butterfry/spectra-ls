...
namespace esphome {
namespace arylic_uart {

class ArylicUart : public Component, public uart::UARTDevice {
 public:
  explicit ArylicUart(uart::UARTComponent *parent) : UARTDevice(parent) {}

  void setup() override { ESP_LOGI("arylic_uart", "Arylic UART initialized"); }

  void dump_config() override { ESP_LOGCONFIG("arylic_uart", "Arylic UART"); }
  ...
  void send_cmd(const std::string &cmd) {
    ESP_LOGI("arylic_uart", "TX %s", cmd.c_str());
  ...
  void set_source(const std::string &source) { this->send_cmd("SRC:" + source + ";"); }

  void set_volume_sensor(sensor::Sensor *sensor) { this->volume_sensor_ = sensor; }

  void set_bass_sensor(sensor::Sensor *sensor) { this->bass_sensor_ = sensor; }

  void set_treble_sensor(sensor::Sensor *sensor) { this->treble_sensor_ = sensor; }

  void set_mute_sensor(binary_sensor::BinarySensor *sensor) { this->mute_sensor_ = sensor; }

  void set_playing_sensor(binary_sensor::BinarySensor *sensor) { this->playing_sensor_ = sensor; }

  void set_source_sensor(text_sensor::TextSensor *sensor) { this->source_sensor_ = sensor; }
 ...
 private:
  ...
  void handle_line_(const std::string &line) {
    if (line.rfind("STA:", 0) == 0) {
      ESP_LOGI("arylic_uart", "RX %s", line.c_str());
